% example11.tex   Author: Akira Hakuta, Date: 2017/06/11
...
\begin{document} 
	...
	\begin{enumerate}	
		...
		\item \pyc{texexpr=r'\frac{\frac{1}{~2~}-\frac{1}{~3~}}{~~\frac{1}{~2~}+\frac{1}{~3~}~~}'}
			\pyc{result=simplify(tex2sym(texexpr))}
			\py{'$\displaystyle '+mylatexstyle(texexpr).replace('\\frac','\\dfrac')+'='+mylatex(result)+'$'}\myvspace
		
		\item \pyc{texexpr=r'-\frac{3}{~5~}a-0.8a+a\div 5\times 8'}
			\pyc{result=simplify(tex2sym(texexpr))}
			\py{'$\displaystyle '+mylatexstyle(texexpr).replace('\\frac','\\dfrac')+'='+mylatex(factor(tex2sym(texexpr)))+'$'}\myvspace
