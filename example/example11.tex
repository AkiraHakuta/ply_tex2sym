% example11.tex   Author: Akira Hakuta, Date: 2017/07/13
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\large\bf ply\_tex2sym example~11~~~No.\thepage}
\fancyfoot{}
...
\begin{document} 
	...
	\begin{enumerate}	
		...
		\item \pyc{texexpr = 'a_{n+1}=3a_{n}+12'}    
			...
			\py{'${:s},~~a_1={:d},~~~~solution:~a_n={:s}$'.format(mylatexstyle(texexpr),a1,mylatex(result))}\myvspace	
		
		\item \pyc{texexpr = 'a_{n+2}=12a_{n+1}-35a_{n}'}    
			\pyc{a1=2; a2=24}  
			\pyc{result=rsolve(simplify(tex2sym(texexpr)),F(n), {F(1):a1,F(2):a2})}
			\py{'${:s},~~a_1={:d},~a_2={:d}~~~~solution:~a_n={:s}$'.format(mylatexstyle(texexpr),a1,a2,mylatex(result))}\myvspace			
		
		\item \pyc{texexpr='2x^2+3x+4=0'}
			\pyc{result=solveset(tex2sym(texexpr), x,  domain=S.Complexes )}
			\py{'${:s},~~~~solution:~\displaystyle {:s}$'.format(mylatexstyle(texexpr),mylatex(result))}\myvspace	
