# tex2sym_parser.py   Author: Akira Hakuta, Date: 2017/07/19
...
# variable : a,b,...,z,A,B,...,D,  F,G,H, J,K,...,R, T,U,..,Z,\\alpha,\\beta,\\gamma,\\theta,\\omega (except E,I,S)
...
if __name__ == '__main__':
    ...
    test('(\\frac{d}{dx})^{3}{x^5}')
    test('\\int{\\sin^{2}{x} dx}')
    test('\\sum_{k=1}^{n}{k^3}')
    test('\\Gamma(x)')
    test('\\zeta(x)')    
    test('2x^2+3x+4=0')
    test('\\dfrac{d^{2}}{dx^{2}}{f(x)}=-f(x)')
    ...
    test('\\left\\{A-2(B-C)\\right\\}^2')
    test('\\int{t^2\\log{t} dt}')    
    test('\\frac{d^{2}}{dx^{2}}{x^5}')        
    test('\\frac{d}{daalpha}{aalpha^3}')
    '''
